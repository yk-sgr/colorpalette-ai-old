...
export default function Color({color}: { color: Color }) {
  return (
    <div key={color.name + color.background + color.foreground} className={"group flex flex-col gap-2"}>
      ...
      <div className={"flex flex-col gap-2"}>
        <div
          ...
          style={{backgroundColor: color.background, boxShadow: `0 6px 8px -6px ${color.background}`}}>
        </div>
        <p
          className={"z-0 text-center font-semibold text-muted-foreground transition duration-100 ease-out group-hover:-translate-y-10 group-hover:transition group-hover:duration-75 group-hover:ease-in"}>{color.background}</p>
      </div>
    </div>
  )
}
