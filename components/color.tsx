    ...
    <div key={color.name + color.hex + color.hex} className={"flex flex-col gap-2"}>
      <div className={"group flex flex-col gap-2"}>
        <div
          ...
          <Text size={"lg"} weight={"semibold"} className={"overflow-hidden text-ellipsis whitespace-nowrap"}>{color.name}</Text>
        </div>
        <div className={"flex flex-col gap-2"}>
          <Link href={`/app/${paletteId}/${color.id}`}>
            <div
              className={`z-10 flex aspect-square cursor-pointer items-center justify-center rounded-xl border border-muted-foreground/20 shadow-md transition duration-150 ease-out active:border-foreground/80 group-hover:-translate-y-3 group-hover:scale-110 group-hover:transition group-hover:duration-100 group-hover:ease-in`}
