    ...
    <div key={color.name + color.hex + color.hex} className={"flex flex-col gap-2"}>
      <div className={"group flex flex-col gap-2"}>
        ...
        <div className={"flex flex-col gap-2"}>
          <div
            className={`z-10 flex h-60 cursor-pointer items-center justify-center rounded-xl border border-muted-foreground/20 shadow-md transition duration-150 ease-out group-hover:-translate-y-3 group-hover:scale-110 group-hover:transition group-hover:duration-100 group-hover:ease-in`}
            style={{backgroundColor: color.hex, boxShadow: `0 6px 8px -6px ${color.hex}`}}>
          </div>
        </div>
      </div>
      <p
        className={"z-0 text-center font-semibold text-muted-foreground"}>{color.hex}</p>
    </div>
  )
}
